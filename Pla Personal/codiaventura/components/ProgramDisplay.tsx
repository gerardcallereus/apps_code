...
import { Trash2, RefreshCw, GripVertical, Minus, Plus, Box, Square, Play, FastForward } from 'lucide-react';
...
const ProgramDisplay: React.FC<ProgramDisplayProps> = ({ 
  commands, onRemove, onRun, onStop, onReset, onClear, onUpdateValue, onReorder, isRunning, activeLineIndex, maxCommands, isFastForward, onToggleSpeed
}) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#2d2d2d] border-8 border-black p-0 shadow-2xl select-none">
      ...
      <div className="p-4 grid grid-cols-5 gap-2 md:gap-4 bg-[#4a4a4a] border-t-8 border-black">
         {!isRunning ? (
             <motion.button 
             ...
             <motion.button 
                ...
                whileTap={{ scale: 0.98 }}
                onClick={onStop}
                className="col-span-2 bg-red-600 text-white p-2 md:p-3 border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] hover:bg-red-500 active:shadow-none active:translate-x-1 active:translate-y-1 font-bold uppercase tracking-widest text-[10px] flex items-center justify-center gap-2"
             >
                <Square size={18} fill="currentColor" /> <span className="hidden md:inline">ATURAR</span>
             </motion.button>
         )}
        ...
        <motion.button 
          ...
          className={`${isFastForward ? 'bg-yellow-400 text-black' : 'bg-slate-600 text-white'} p-2 md:p-3 border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] hover:bg-yellow-300 active:shadow-none active:translate-x-1 active:translate-y-1 font-bold flex items-center justify-center`}
          title="Velocitat x2"
        >
          <FastForward size={20} fill={isFastForward ? "currentColor" : "none"} />
        </motion.button>
        ...
        <motion.button 
          ...
          onClick={onReset}
          disabled={isRunning}
          className="bg-blue-500 text-white p-2 md:p-3 border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] hover:bg-blue-400 active:shadow-none active:translate-x-1 active:translate-y-1 font-bold disabled:opacity-50 flex items-center justify-center"
        >
          <RefreshCw size={20} />
        </motion.button>
        <motion.button 
          ...
          onClick={onClear}
          disabled={isRunning}
          className="bg-red-500 text-white p-2 md:p-3 border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] hover:bg-red-400 active:shadow-none active:translate-x-1 active:translate-y-1 font-bold disabled:opacity-50 flex items-center justify-center"
        >
          <Trash2 size={20} />
        </motion.button>
      </div>
